...
% addpath('./lib')
% Tiff名称
Tiffname={'Altitude','Aspect','Geology','Historic','Landcover','Landform','Sloper','Soilscape','Vegetation'};
...
for i=1:numtiff
    [Igroup{i},~]=geotiffread(['./Raw_data_30m/Raw_data_30m/' Tiffname{i} '.tif']);
end
info = geotiffinfo(['./Raw_data_30m/Raw_data_30m/' Tiffname{4} '.tif']);  % 读取tif数据的地理信息，为后面导出为tif数据提供地理信息
[temp,R]=geotiffread(['./Raw_data_30m/Raw_data_30m/' Tiffname{4} '.tif']);
[height_src,width_src,~]=size(temp);

[labelmap,~]=geotiffread(['./Raw_data_30m/Raw_data_30m/Manual_LDUs.tif']);
...
[experts]=xlsread('Multi-view.xlsx', 'Sheet1', 'B2:J11');
...
savenametif=[resPath 'results_1-dif1.tif'];
