...
class SwinTransformerPolar(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, radius_cuts=16, azimuth_cuts = 64, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, distortion_model = 'spherical', n_radius=10, n_azimuth=10, P = 16, **kwargs):
        super().__init__()
    ...
    def forward_features(self, x, dist):
        ...
        x = self.norm(x)  # B L C
        x_ = self.avgpool(x.transpose(1, 2))  # B C 1
        x_ = torch.flatten(x_, 1)
        return x_, x

    def forward(self, x, dist):
        x_, x = self.forward_features(x, dist)
        x_ = self.head(x_)
        # import pdb;pdb.set_trace()
        return x_
...
if __name__=='__main__':
    model = SwinTransformerPolar(img_size=64,
                        radius_cuts=16, 
                        azimuth_cuts=64,
                        in_chans=3,
                        num_classes=200,
